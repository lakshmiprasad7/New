...
def search_extracted_text(text, search_term):
  """
  Searches for the given search term within the extracted text.
  ...
  Returns:
      A list of strings containing the search results (occurrences of the search term).
  """
  if search_term:
    search_term = search_term.lower()  # Make search case-insensitive
    return [line for line in text.splitlines() if search_term in line.lower()]
  else:
    return []
...
def main():
    ...
    if uploaded_file is not None:
        if uploaded_file.type == 'application/pdf':
            ...
            # Add search bar
            search_term = st.text_input("Search within extracted text:")
            # PDF file
            ...
            if st.button("Extract Text and Generate Metadata (PDF)"):
                with st.spinner('Processing...'):
                    ...
                    search_results = search_extracted_text(extracted_text, search_term)
                    ...
                    if search_results:
                        ...
                        st.write("No matches found for your search term")
